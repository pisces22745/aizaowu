<template>
  <div id="registe">
    ...
    <div class="input-wrapper check-code clearfix">
      ...
      <button class="check-code-msg fr" :class="{disabled: checkCodeFlag}" @click="getCheckCode">{{checkCodeMsg}}</button>
    </div>
    <div class="input-wrapper">
    ...
    <div class="login" @click="login=true">登录</div>
  </div>
</template>
<script>
  export default {
    ...
    methods:{
      getCheckCode(){
        if(!this.checkCodeFlag){
          let mobile_reg = /^(13|14|15|17|18)[0-9]{9}$/
          let _this = this
